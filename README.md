...
Current Milestone: Expanded Full-Stack Skill Catalog ✅
...
Recognize 31 frontend, backend, cloud, testing, database, and DevOps skills
...
The analyzer also joins wrapped lines inside the same resume paragraph. This allows it to understand text such as:

4 years of
React experience

as one experience statement.
...
The current skill catalog recognizes:
...
The user enters resume text.

The user pastes a job description.

CareerForge divides the job description into general, required, and preferred sections.

The analyzer loads recognized skills and aliases from src/data/skills.ts.

The analyzer searches the job-description sections for recognized skills and aliases.

Skills found in general or required sections are classified as required.

Skills found in preferred sections are classified as preferred.

If a skill appears in both categories, it is treated as required.

CareerForge checks the resume for each identified skill.

The analyzer detects explicit years-of-experience requirements.

Wrapped resume lines inside the same paragraph are joined before experience evaluation.

The resume is checked for explicitly stated years associated with those requirements.

CareerForge calculates:
...
Dynamic recommendations are generated from the results.

The results are displayed in an interactive dashboard.
...
Handles experience statements that wrap across multiple lines
...
Prevents new skill names from matching inside unrelated words
...
Testing aliases across frontend, backend, cloud, database, and DevOps tools
